<template>
  ...
  <div class="dance-music-search-detail">
    <div class="search-detail-title">
      ...
      /> -->
      <el-tabs v-model="activeName" @tab-click="handleChildrenRouter">
        <el-tab-pane v-for="(item,index) in menuList" :key="index" :label="item.content" :name="item.content"></el-tab-pane>
      </el-tabs>
    </div>
...
export default {
  ...
  methods: {
    /**子路由 */
    handleChildrenRouter(tab) {
      // this.forword()
      console.log(menuList[tab.index].link);
      this.$router.push({
        path:this.menuList[tab.index].link,
        query:{
